class CalendarsController < BaseController
  ...
  def index
    ...
    selected_calendar_ids = (pcids.empty? ? @calendars.map(&:id) : @calendars.select { |c| pcids.include?(c.id) }.map(&:id)).compact
  ...
  def set_calendars
    ...
    if current_organization.slug == "medgical"
      ...
      @calendars << medgical_calendar if medgical_calendar.present?
    end
