...
class PipelinesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pipeline" do
    assert_difference("Pipeline.count") do
      post pipelines_url, params: { pipeline: { name: @pipeline.name, organization_id: @pipeline.organization_id } }
    end
  ...
  test "should update pipeline" do
    patch pipeline_url(@pipeline), params: { pipeline: { name: @pipeline.name, organization_id: @pipeline.organization_id } }
